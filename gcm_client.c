...
int pt_len;
unsigned char enbuf[1048576];
unsigned char debuf[1048576];
unsigned char tagbuf[1024];
...
//        printf("gcm_pt size: %d\n",pt_len);
        ...
        /* Encrypt plaintext */
        EVP_EncryptUpdate(ctx, enbuf, &outlen, gcm_pt, pt_len);
        /* Output encrypted block */
//        printf("Ciphertext:\n");  
//        BIO_dump_fp(stdout, outbuf, outlen);  
        /* Finalise: note get no output for GCM */
        EVP_EncryptFinal_ex(ctx, enbuf, &outlen);
        /* Get tag */
...
//        BIO_dump_fp(stdout, gcm_ct, sizeof(gcm_ct));  
        ...
        /* Decrypt plaintext */  
        EVP_DecryptUpdate(ctx, debuf, &outlen, retrieved_value, value_length);  
...
//        BIO_dump_fp(stdout, outbuf, outlen);  
        ...
        /* Finalise: note get no output for GCM */  
        rv = EVP_DecryptFinal_ex(ctx, debuf, &outlen);  
...
//value plain text length
  ...
  gcm_pt = plain;
  printf("value plain text:");
  BIO_dump_fp(stdout, gcm_pt, pt_len);

  RDTSC_LL(tStart);

  //enbuf stores the encrypted value, debuf stores the decrypted value
  aes_gcm_encrypt();

  rc = memcached_set(memc, key, strlen(key), enbuf, pt_len, (time_t)0, (uint32_t)0);
  ...
  RDTSC_LL(tEnd);
  printf("Get KV time: %lf us\n",(tEnd-tStart)/2600.0);
  printf("Received value:\n");
  BIO_dump_fp(stdout, retrieved_value, value_length);
